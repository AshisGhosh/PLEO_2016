...
forward public love_init();
forward public love_eval();
forward public love_behavior_eval(behavior_id);
forward public love_exit();
forward public love_activate();
forward public love_deactivate();
...
public love_init()
{
	...
	behavior_add(scr_catcall, "love", 0, 100, 100, 100, 1000);
    behavior_add(scr_inlove, "love", 0, 100, 100, 100, 1000);
...
public love_eval()
{
    ...
    if (property_get(property_lovepoints) >= 30)
    {
        return 100;
    }
...
//
public love_behavior_eval(behavior_id)
{
    ...
    switch (behavior_id)
    {
		...
		case scr_catcall:
        {
            if (property_get(property_lovepoints) < 60){
                sound_play(snd_whistle);
                //while(sound_is_playing(snd_whistle)){}
                return 50;
            }
        ...
        {
            if (property_get(property_lovepoints) >= 60){
                sound_play(snd_lovesong);
                //while(sound_is_playing(snd_lovesong)){}
                return 100;
            }
        }
